...
from core.utils.mixed_datasets import LoadImages, LoadStreams
from core.utils.general import (apply_classifier, check_img_size, check_imshow,
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    stride, de_names = 64, [f'class{i}' for i in range(1000)]  # assign defaults
    if pt:
        ...
        for i, (det, seg) in enumerate(zip(pred, seg_pred)):  # per image
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    s += f"{n} {de_names[int(c)]}{'s' * (n > 1)}, "  # add to string
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        label = None if hide_labels else (de_names[c] if hide_conf else f'{de_names[c]} {conf:.2f}')
