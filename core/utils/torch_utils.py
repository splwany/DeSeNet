...
def select_device(device='', batch_size=None) -> torch.device:
...
def intersect_dicts(da, db, exclude=()) -> OrderedDict:
    # 字典取交集，匹配 key 和 shapes, 忽略 exclude 中的 keys, 使用 da 中的值
    od = OrderedDict()
    for k, v in da.items():
...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, updates=0):
        ...
        # 创建 EMA
        self.ema: nn.Module = deepcopy(de_parallel(model)).eval()  # FP32 EMA
