...
def fitness_det_seg(x, mIoU):
    # Model fitness as a weighted combination of metrics
    w = [0.0, 0.0, 0.1, 0.2, 0.7]  # weights for [P, R, mAP@0.5, mAP@0.5:0.95, mIoU]
    # print(x)
    x_m = np.expand_dims(np.append(x[:, :4], mIoU), 0)  # x 在 train.py 有 reshape, 确定只有 1 行
    return (x_m * w).sum(1)
...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):
        ...
        for i, gc in enumerate(gt_classes):
            j = m0 == i
            if n and sum(j) == 1:
                self._matrix[detection_classes[m1[j]], gc] += 1  # correct
            else:
                self._matrix[self.nc, gc] += 1  # background FP

        if n:
            for i, dc in enumerate(detection_classes):
                if not any(m1 == i):
                    self._matrix[dc, self.nc] += 1  # background FN
    ...
    def plot(self, normalize=True, save_dir='', names=()):
        try:
            ...
            array = self._matrix / ((self.matrix.sum(0).reshape(1, -1) + 1E-6) if normalize else 1)  # normalize columns
    ...
    def print(self):
        for i in range(self.nc + 1):
            print(' '.join(map(str, self._matrix[i])))
