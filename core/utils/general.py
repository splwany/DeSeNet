...
class Profile(contextlib.ContextDecorator):
    # 使用方法：@Profile() decorator or 'with Profile():' context manager
    def __enter__(self):
...
class Timeout(contextlib.ContextDecorator):
    # 只支持 Linux 系统，Windows 不起作用
    # 使用方法：@Timeout(seconds) decorator or 'with Timeout(seconds):' context manager
...
def try_except(func):
    # try-except 函数. 使用方法：@try_except decorator
...
def check_git_status():
    ...
    msg = ', for updates see https://github.com/splwany/DeSeNet'
    ...
    if n > 0:
        s = f"⚠️ DeSeNet is out of date by {n} commit{'s' * (n > 1)}. Use `git pull` or `git clone {url}` to update."
    else:
...
def check_file(file, suffix=''):
    ...
    if Path(file).is_file() or file == '':  # exists
        ...
        file = Path(unquote(file)).name.split('?')[0]  # '%2F' to '/', split https://url.com/file.txt?auth
...
def generate_seg_labels_img(seg_labels, img_hw) -> torch.Tensor:
    # 根据一张图的语义分割点坐标的归一化值和图片尺寸，生成一张uint8的图
    cls = np.zeros(img_hw, dtype=np.int32)
    for shape in seg_labels:
        cls_id, points = shape
...
def seg_xyn2xy(x, w=640, h=640, padw=0, padh=0):
    y = [item.copy() for item in x]
    for item in y:
        if len(item):
            item[:, 0] = w * item[:, 0] + padw
            item[:, 1] = h * item[:, 1] + padh
    return y
...
def print_mutation(results, hyp, save_dir, bucket):
    ...
    with open(evolve_yaml, 'w') as f:
        data = pd.read_csv(str(evolve_csv))
