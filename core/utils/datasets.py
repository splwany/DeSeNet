...
class InfiniteDataLoader(torch_data.dataloader.DataLoader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._batch_sampler = _RepeatSampler(self.batch_sampler)
        self.iterator = super().__iter__()

    def __len__(self):
        return len(self._batch_sampler.sampler)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            self.img_files = sorted([str(x) for x in f if x.suffix[1:].lower() in img_formats])  # pathlib
...
def load_image(self, index: int) -> Tuple[np.ndarray, Tuple[int, int], Tuple[int, int]]:
    ...
    assert isinstance(self.imgs, list)
    img = self.imgs[index]
    if img is None:  # not cached
        ...
        return img, (h0, w0), (img.shape[0], img.shape[1])  # img, hw_original, hw_resized
    else:
        ...
        if im_file.suffix[1:] in img_formats:
            ...
            if Path(lb_file).exists():
                ...
                for j, x in enumerate(lb):
                    ...
                    b[2:] = b[2:] * 1.2 + 3  # pad
                    xyxy = xywh2xyxy(b.reshape(-1, 4))
                    assert isinstance(xyxy, np.ndarray)
                    b = xyxy.ravel().astype(np.int32)
